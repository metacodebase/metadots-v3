...
export default function CaseStudyCard({ caseStudy }: CaseStudyCardProps) {
  ...
  return (
    <Card className="group relative overflow-hidden border-0 shadow-xl hover:shadow-2xl transition-all duration-500 transform hover:scale-105 bg-white h-full flex flex-col">
      <div className="relative flex-shrink-0">
        <div
          className={`aspect-video bg-gradient-to-br ${getGradientColor(
            caseStudy.industry
          )} relative overflow-hidden`}>
          <Image
            src={
              caseStudy.heroImage ||
              "https://www.knowbe4.com/hubfs/Its%20Not%20About%20URL.jpg"
            }
            alt={caseStudy.title}
            fill
      ...
      <CardHeader className="p-6 flex-1 flex flex-col">
        ...
        <div className="flex-shrink-0 mb-4">
          ...
          <CardTitle className="text-xl mb-3 text-blue-600 transition-colors leading-tight min-h-[3.5rem] flex items-start">
            {caseStudy.title}
          </CardTitle>

          {caseStudy.subtitle && (
            <p className="text-sm text-slate-500 mb-3 italic font-medium min-h-[1.25rem] leading-tight">
              {caseStudy.subtitle}
            </p>
          )}
        </div>

        {/* Description Section - Flexible but Consistent Height */}
        <div className="flex-1 mb-4">
          <CardDescription className="text-base leading-relaxed line-clamp-3 min-h-[4.5rem] flex items-start">
            {caseStudy.description}
          </CardDescription>
        </div>

        {/* Results Section - Fixed Height */}
        <div className="flex-shrink-0 mb-4">
          <div className="space-y-2 h-[80px] flex flex-col justify-start">
        ...
        <div className="flex-shrink-0 mt-auto">
          {/* Project Details */}
          <div className="flex items-center justify-between font-semibold gap-2 text-xs text-slate-500 mb-4 min-h-[1rem]">
            <div>
